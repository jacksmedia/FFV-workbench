;=;=;=;=;=;=;=;=;=;=;=;=;=;=;=;=;=;=;=;=;=;=;=;=;=;=;=;=;=;=;=;=;=;=;=;=;=;=;=;=
; Cid canon sprites FFVJ - xJ4cks .ips.commented.asm
;    Disassembled from IPS patch: Cid canon sprites FFVJ - xJ4cks .ips
...
org $DAB850
    ...
    COP #$2C                ; [02 2C      ] Coprocessor interrupt
...
;--- Patch at file offset $1AB879 (SNES $DAB879) ---
org $DAB879
...
org $DAB8B7
    ...
    ORA ($6F,X)             ; [01 6F      ] OR A with (DP,X)

;--- Patch at file offset $1AB8D0 (SNES $DAB8D0) ---
org $DAB8D0

    ORA $10,S               ; [03 10      ] OR A with Stack Relative
    BRK                     ; [00 2F      ] Software interrupt

;--- Patch at file offset $1AB8E0 (SNES $DAB8E0) ---
org $DAB8E0

    INC $80,X               ; [F6 80      ] Increment DP,X
    ...
    ROL $E010,X             ; [3E 10 E0   ] Rotate Left Absolute,X
...
org $DAB960
    ...
    RTI                     ; [40         ] Return from Interrupt
    JSR $E000               ; [20 00 E0   ] Jump to Subroutine
    BRK                     ; [00 30      ] Software interrupt
    BRK                     ; [00 30      ] Software interrupt
    BRK                     ; [00 00      ] Software interrupt
    BRK                     ; [00 00      ] Software interrupt
    BRK                     ; [00 C0      ] Software interrupt
    BRA $B940               ; [80 C0      ] Branch Always
    ORA $001300             ; [0F 00 13 00] OR A with Absolute Long
    JSR $1408               ; [20 08 14   ] Jump to Subroutine
    ORA $160F18             ; [0F 18 0F 16] OR A with Absolute Long
...
org $DAB9D0
    ...
    BRK                     ; [00 00      ] Software interrupt
    ORA $180C00             ; [0F 00 0C 18] OR A with Absolute Long
    TRB $0404               ; [1C 04 04   ] Test and Reset Bits Absolute
    SEI                     ; [78         ] Set Interrupt Disable
    BRA $B99B               ; [80 B8      ] Branch Always

;--- Patch at file offset $1AB9F3 (SNES $DAB9F3) ---
org $DAB9F3

    BCS $B9F5               ; [B0 00      ] Branch if Carry Set
...
org $DABB16
    ...
    BMI $BB24               ; [30 00      ] Branch if Minus
    STZ $1900,X             ; [9E 00 19   ] Store Zero to Absolute,X
...
;--- Patch at file offset $1ABB79 (SNES $DABB79) ---
org $DABB79
