"""Analyze how many NPC switches can be named by cross-referencing data."""
import json
...
# Existing named NPC switches from ff5u-sfc.json (hand-counted from stringTable)
NAMED_IDS = set()
with open(r"C:\Users\J4cks\Desktop\work5\crawler\tools\ff5\ff5u-sfc.json", encoding='utf-8') as f:
    jdata = json.load(f)
ns_strings = jdata['stringTable']['npcSwitch']['string']
for k in ns_strings:
    NAMED_IDS.add(int(k))
...
def parse_npc_script_dialog(rom, npc_id):
    """Extract the fallback dialog ID from an NPC script (the 2-byte value after trigger opcodes)."""
    ...
    # Walk past trigger opcodes to find the fallback dialog bytes
    events_called = []
    i = file_start
    while i < file_end:
        b = rom[i]
        if b == 0xF0: i += 1; break  # End opcode
        elif b == 0xFF and i + 2 < file_end:
            eid = rom[i+1] | (rom[i+2] << 8)
            events_called.append(eid)
            i += 3
        elif b == 0xF5: i += 5
        elif b == 0xF6: i += 1
        elif b == 0xF7: i += 2
        ...
        elif 0xFB <= b <= 0xFE: i += 2
        else: break  # Hit fallback dialog bytes

    # Read 2-byte fallback dialog ID (if present)
    dialog_id = None
    dialog_text = None
    if i + 1 < file_end:
        raw = rom[i] | (rom[i+1] << 8)
        dialog_id = raw & 0x7FFF
        if dialog_id < 2160:
            dialog_text = decode_dialog(rom, dialog_id)

    return {
        'events': events_called,
        'dialog_id': dialog_id,
        'dialog_text': dialog_text,
        'script_size': file_end - file_start,
    }
...
def parse_npc_properties(rom):
    ...
    npc_to_maps = {}  # npc_script_id -> [(map_id, map_name, x, y, gfx)]
...
def main():
    ...
    print(f"=== NPC Switch Naming Feasibility Analysis ===\n")
    print(f"Total NPC switches/scripts: {NUM_NPC_SCRIPTS}")
    print(f"Already named in JSON: {len(NAMED_IDS)}")
    print(f"Unnamed: {NUM_NPC_SCRIPTS - len(NAMED_IDS)}\n")

    # Parse NPC properties for map placement
    npc_to_maps = parse_npc_properties(rom)

    # Analyze each unnamed switch
    has_map = 0
    has_dialog = 0
    has_events = 0
    has_map_and_dialog = 0
    no_data_at_all = 0
    orphan_ids = []  # Not on any map

    sample_nameable = []  # Collect samples where we can produce good names

    for npc_id in range(NUM_NPC_SCRIPTS):
        if npc_id in NAMED_IDS:
            continue

        on_map = npc_id in npc_to_maps
        script_info = parse_npc_script_dialog(rom, npc_id)
        has_dlg = script_info['dialog_text'] is not None and len(script_info['dialog_text']) > 2
        has_evt = len(script_info['events']) > 0

        if on_map: has_map += 1
        if has_dlg: has_dialog += 1
        if has_evt: has_events += 1
        if on_map and has_dlg: has_map_and_dialog += 1
        if not on_map and not has_dlg and not has_evt:
            no_data_at_all += 1
            orphan_ids.append(npc_id)

        # Collect samples
        if on_map and has_dlg and len(sample_nameable) < 20:
            maps = npc_to_maps[npc_id]
            sample_nameable.append({
                'id': npc_id,
                'map': maps[0]['map_name'],
                'dialog': script_info['dialog_text'][:50],
                'events': script_info['events'][:3],
            })

    unnamed = NUM_NPC_SCRIPTS - len(NAMED_IDS)
    print(f"--- Unnamed NPC Switch Analysis ({unnamed} switches) ---")
    print(f"  Have map placement:           {has_map:4d} ({has_map*100//unnamed}%)")
    print(f"  Have NPC dialog text:         {has_dialog:4d} ({has_dialog*100//unnamed}%)")
    print(f"  Have events called:           {has_events:4d} ({has_events*100//unnamed}%)")
    print(f"  Have BOTH map + dialog:       {has_map_and_dialog:4d} ({has_map_and_dialog*100//unnamed}%) <-- best candidates")
    print(f"  No data at all (orphaned):    {no_data_at_all:4d} ({no_data_at_all*100//unnamed}%)")

    print(f"\n--- Sample Auto-Nameable Entries ---")
    for s in sample_nameable:
        proposed = f'{s["map"]} - "{s["dialog"][:30]}"'
        print(f"  NPC {s['id']:3d}: {proposed}")

    print(f"\n--- Orphan IDs (no map, no dialog, no events) ---")
    if orphan_ids:
        # Show first 30
        print(f"  {orphan_ids[:30]}")
        if len(orphan_ids) > 30:
            print(f"  ... and {len(orphan_ids) - 30} more")
